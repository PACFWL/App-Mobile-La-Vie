...
const OrderScreen = ({ navigation }) => {
  ...
  return (
    <View style={styles.container}>
      <NavigationHeader text="Seu pedido" logoUrl={restaurant.logoUrl} />

      <FlatList
        data={items}
        renderItem={({ item }) => <OrderItem item={item} />}
        keyExtractor={item => item.item.name}
        ListEmptyComponent={() => <Text style={styles.emptyText}>Não há itens no seu pedido</Text>}
        showsVerticalScrollIndicator={false}
        style={styles.orderList}
      />

      <OrderResumeCTA text="Pedido" total={total} navigateTo="Checkout" itemsLength={items.length} />
    </View>
  );
}
