...
const tags = [
  { label: 'Café da manhã', icon: 'coffee-outline' },
  { label: 'Almoço', icon: 'food-outline' },
  { label: 'Janta', icon: 'food-steak' },
  { label: 'Lanche', icon: 'food-croissant' },
  { label: 'Sobremesa', icon: 'ice-cream' }
]
...
const HomeScreen = ({ navigation }) => {
  ...
  const [selectedTag, setSelectedTag] = useState('Café da manhã');
  ...
  useEffect(() => {
    dispatch(filterRestaurantsByTag({ categories: selectedTag }));
    setSearch('');
  ...
  return (
    <DismissKeyboardView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerSubtitle}>Bem vindo {getFirstName(user.fullName)},</Text>
        <Text style={styles.headerTitle}>Aproveite a sua refeição!</Text>
      </View>

      <SearchBar placeholder="Encontrar restaurantes" search={search} setSearch={setSearch} />
