...
const restaurantsSlice = createSlice({
  ...
  reducers: {
    filterRestaurantsByTag: (state, action) => {
      ...
      state.filteredItems = state.items.filter(restaurant => restaurant.categories.includes(categories))
    },
  ...
  extraReducers: builder => {
    builder.addCase(fetchRestaurants.pending, state => {
      ...
      builder.addCase(fetchRestaurants.fulfilled, (state, action) => {
        ...
        state.filteredItems = action.payload.filter(restaurant => restaurant.categories.includes('Café da manhã'));
        state.loading = false;
      }),
